...
For inbound email, the Receive connector *MaxRecipientsPerMessage* is verified first. However, if the number of recipients exceeds the limit, the message is not rejected; the connection receives the error, `452 4.5.3 Too many recipients`. Most mail servers understand this error and they will continue to resend the message in another connection until the message is delivered to all recipients. Apart from anonymous submissions, the Receive connector *MaxRecipientsPerMessage* also applies to authenticated SMTP client submissions. However, when an Exchange Server relays email through another Exchange server in the same organization, the Receive connector *MaxRecipientsPerMessage* is bypassed.
