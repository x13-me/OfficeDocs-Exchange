...
|n/a  <br/> | `OutgoingRate` <br/> |Equals (`-eq`)  <br/> Does not equal (`-ne`)  <br/> Greater than (`-gt`)  <br/> Greater than or equal to (`-ge`)  <br/> Less than (`-lt`)  <br/> Less than or equal to (`-le`)  <br/> |A calculated number that indicates how quickly messages are leaving the queue. For more information, see [IncomingRate, OutgoingRate, and Velocity](queues.md#incomingrate-outgoingrate-and-velocity).  <br/> |
