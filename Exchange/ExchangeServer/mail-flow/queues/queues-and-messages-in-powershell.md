...
|**Redirect-Message** <br/> | _Server_ <br/> |This cmdlet drains active messages from all delivery queues on the specified server, so _Server_ is the only filtering parameter that's available. For more information, see [Redirect messages in queues](message-procedures.md#redirect-messages-in-queues).  <br/> |
