...
|Memory|Varies depending on Exchange roles that are installed: <br/>• **Mailbox**: 8GB minimum <br/>• **Edge Transport**: 4GB minimum  <br/> Note that Exchange 2019 has large memory support (up to 256 GB).||
|Paging file size|Set the page file size minimum and maximum to the amount of physical RAM plus 10MB, up to a maximum size of 32,778MB (32GB) if you're using more than 32GB of RAM.|None|
...
- Installing Exchange 2019 on a computer that's running Windows Server Core is fully supported. The Desktop Experience feature is no longer required.
