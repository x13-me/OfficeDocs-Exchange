...
The requirements to install Exchange 2019 on computers running Windows Server 2019 are described in the following sections.  Multiple options to install pre-requisites exist.  The easiest and recommend option is to simply use the /InstallWindowsComponents when invoking setup from the command line. The graphical setup program provides the option to Install Windows Prerequisites automatically as well.  Using either option you will no longer be required to restart your computer after the Windows components are added.  The option to specify individual features as specified below is also available.  
