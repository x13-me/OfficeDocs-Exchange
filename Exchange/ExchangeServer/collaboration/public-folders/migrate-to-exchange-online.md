...
```
> [!NOTE]
>The map.csv generated by the script uses generic names (like Mailbox1, Mailbox2) for the taget public folder mailboxes that will be created in EXO during the next step. You are encouraged to change the public folder mailbox names in the map.csv to suit your organization's naming policies. Also, if your on-premises organization already has mailboxes named Mailbox1, Mailbox2, you should edit the map.csv and provide unique names for target public folder mailboxes in EXO. Please use Notepad or similar editor to edit the TargetMailbox names in the map.csv
