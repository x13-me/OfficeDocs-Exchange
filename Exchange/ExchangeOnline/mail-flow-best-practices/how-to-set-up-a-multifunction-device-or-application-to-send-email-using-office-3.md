...
- **Authentication**: You must be able to configure a user name and password to send email on the device. Note that because basic authentication is required, you cannot use [Microsoft Security Defaults](https://docs.microsoft.com/azure/active-directory/fundamentals/concept-fundamentals-security-defaults), which are designed to protect your users from compromise. If at all possble, we recommend using Option 2 or 3 below.
