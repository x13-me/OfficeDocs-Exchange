...
- **Authentication**: You must be able to configure a user name and password to send email on the device. Also, because basic authentication is required, you cannot use [Microsoft Security Defaults](https://docs.microsoft.com/azure/active-directory/fundamentals/concept-fundamentals-security-defaults), which can protect the rest of your users from compromise. For this reason, we recommend using Option 2 or 3 below, if at all possible.
