...
|5.1.10|`Recipient Not Found`| Recipient <SMTP Address> not found by SMTP address lookup. See [Fix email delivery issues for error code 550 5.1.10 in Exchange Online](https://docs.microsoft.com/en-us/exchange/mail-flow-best-practices/non-delivery-reports-in-exchange-online/fix-error-code-550-5-1-10-in-exchange-online) for help on fixing this issue.|
...
|5.4.300|`Message Expired`|The email took to long to be successfully delivered, either becuase the destination server never responded or the sent message generated an NDR error and that NDR could not be delivered back to the original sender.|
