...
|**Any attachment is greater than or equal to** <br/><br/> **Any attachment** \> **size is greater than or equal to**|_AttachmentSizeOver_|This condition matches messages with attachments when those attachments are greater than or equal to the size you specify.<br/>**Note:** This rule checks the sizes of individual attachments, not the cumulative size. For example, if this rule is set to reject any attachment that is 10MB or greater, a single attachment with a size of 15MB will be rejected, but a message with three 5 MB attachments will be allowed.  |
