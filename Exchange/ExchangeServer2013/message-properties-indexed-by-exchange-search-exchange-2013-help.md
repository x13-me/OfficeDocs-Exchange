...
**Notes about indexed properties**:
  ...
  - **Queryable properties** can be used in AQS queries by search clients such as Outlook Web App in `property:value` pairs, for example, `from:bsuneja@cotoso.com`. A subset of the queryable properties listed in the previous table can also be used in search queries for In-Place eDiscovery. For a list of these properties, see [Message properties and search operators for In-Place eDiscovery](./message-properties-and-search-operators-for-/exchange/security-and-compliance/in-place-ediscovery/in-place-ediscovery).
