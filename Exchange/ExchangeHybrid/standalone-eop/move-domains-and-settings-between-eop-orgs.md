...
ms.custom: 
  - seo-marvel-apr2020
description: Admins can learn how to move domains and settings from one Exchange Online Protection (EOP) organization to another.
ms.technology: mdo
...
Changing business requirements can sometimes require splitting one organization into two separate organizations, merging two organizations into one, or moving your domains and Exchange Online Protection (EOP) settings from one organization to another organization. Moving from one EOP organization to a second standalone EOP organization can be challenging, but with a few basic PowerShell scripts and a small amount of preparation, you can achieve this with a relatively small maintenance window.

**Notes**:

- Moving domains and settings between EOP organizations is supported in the following scenarios:
  - Standalone EOP organizations to other standalone EOP organizations (also known as EOP standard).
  - EOP standard to EOP that's included in Exchange Enterprise CAL with Services licenses in on-premises Exchange (also known as EOP premium).
  - EOP premium organizations to EOP premium organizations.
- Moving domains and settings between EOP organizations is not supported in the following scenarios:
  - EOP premium organizations to EOP standard organizations (some EOP premium features aren't available in EOP standard organizations).
  - Exchange Online organizations (many additional considerations that are out of scope for the instructions in this article).
...
For more information about configuring DNS, see [Create DNS records at any DNS hosting provider for Microsoft 365](/microsoft-365/admin/get-help-with-domains/create-dns-records-at-any-dns-hosting-provider).
